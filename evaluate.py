...
性能优化：
- 使用torch.inference_mode()替代torch.no_grad()以获得更好的性能
- 支持自动混合精度(AMP)加速推理
- 使用channels_last内存格式优化卷积操作
- 批量处理减少数据传输开销
"""
...
@torch.inference_mode()  # 装饰器：禁用梯度计算，节省内存并加速推理（比no_grad更高效）
def evaluate(net, dataloader, device, amp):
    ...
    Args:
        ...
        dataloader (torch.utils.data.DataLoader): 验证集数据加载器
            ...
            - 'mask': 真实分割标签，形状为(batch_size, height, width)或(batch_size, 1, height, width)
        device (torch.device): 计算设备
            - 支持'cuda'、'cpu'、'mps'等设备类型
            - 用于将数据移动到相应的计算设备上
        amp (bool): 是否启用自动混合精度推理
    ...
    注意事项：
        - 函数会自动处理不同维度的mask输入
        - 多分类任务会自动忽略背景类（第0类）
        - 使用channels_last内存格式可以提升卷积性能
        - MPS设备需要特殊处理autocast设置
    """
    # 将模型设置为评估模式
    ...
    # 混合精度可以显著减少显存占用（约50%）并加速推理（约2-3倍）
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        # 遍历验证集的所有批次数据
        # tqdm提供进度条显示，方便监控评估进度
        # total: 总批次数，用于计算进度百分比
        # desc: 进度条描述文字
        # unit: 进度单位
        # leave=False: 评估完成后不保留进度条，保持终端整洁
        for batch in tqdm(dataloader, total=num_val_batches, desc='验证轮次', unit='batch', leave=False):
            ...
            # image: 转换为float32类型，使用channels_last内存格式优化卷积性能
            #        channels_last格式将通道维度放在最后，更符合现代GPU的内存访问模式
            # mask_true: 转换为long类型（int64），因为标签是整数索引，用于交叉熵和one-hot编码
            # non_blocking=True可以异步传输数据，但这里为了代码简洁性未使用
            image = image.to(device=device, dtype=torch.float32, memory_format=torch.channels_last)
            mask_true = mask_true.to(device=device, dtype=torch.long)

            # 模型前向传播，得到预测结果
            # 输出是原始logits（未归一化的分数），形状为(batch_size, n_classes, height, width)
            # 对于二分类任务，n_classes=1；对于多分类任务，n_classes>1
            # logits不经过softmax/sigmoid，因为损失函数会内部处理
            mask_pred = net(image)

            # 根据分类任务类型计算Dice系数
            if net.n_classes == 1:
                # ==================== 二分类任务处理 ====================
                # 适用场景：前景/背景分割，如医学图像中的病灶检测
                
                # 验证真实标签的值是否在有效范围内[0, 1]
                # 0表示背景，1表示前景
                assert mask_true.min() >= 0 and mask_true.max() <= 1, \
                    f'真实掩码的索引应该在[0, 1]范围内，但得到范围[{mask_true.min()}, {mask_true.max()}]'
                
                # 将logits转换为概率并应用阈值进行二值化
                # 步骤1: sigmoid将输出映射到[0,1]区间，表示像素属于前景的概率
                # 步骤2: 阈值0.5用于二值化决策（概率>0.5判定为前景）
                # 步骤3: 转换为float类型以便后续计算
                # 结果形状：(batch_size, 1, height, width)
                mask_pred = (F.sigmoid(mask_pred) > 0.5).float()
                
                # 计算二分类Dice系数得分
                # reduce_batch_first=False表示先对空间维度归约，再对批次维度求平均
                # 这样可以得到每个样本的Dice得分，然后取平均
                dice_score += dice_coeff(mask_pred, mask_true, reduce_batch_first=False)
            else:
                # ==================== 多分类任务处理 ====================
                # 适用场景：多个类别的前景分割，如多器官分割、多类型细胞分割
                
                # 验证真实标签的值是否在有效范围内[0, n_classes)
                # 例如：3分类任务的标签值应该是0, 1, 2
                assert mask_true.min() >= 0 and mask_true.max() < net.n_classes, \
                    f'真实掩码的索引应该在[0, {net.n_classes})范围内，但得到范围[{mask_true.min()}, {mask_true.max()}]'
                
                # 将真实标签转换为one-hot编码格式
                # 确保mask_true是正确的维度：(batch_size, height, width)
                # 有些数据加载器可能返回4维张量(batch_size, 1, height, width)
                if mask_true.dim() == 4:  
                    # 如果是4维，去掉多余的通道维度
                    # squeeze(1)移除第1维（通道维度），前提是该维度大小为1
    ...
    # 当验证集为空时，返回0.0
    # 使用.item()将张量转换为Python标量（如果dice_score是张量）
